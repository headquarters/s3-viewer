...
// TODO: fetch api/s3, then change this file to +page.ts to run on client and server
export async function load({ url, fetch }: PageServerLoadEvent): Promise<LoadResponse | null> {
  const path = url.searchParams.get('path') as string;
  const region = url.searchParams.get('region') as string;
  const token = url.searchParams.get('token') as string;

  console.log({ path, region, token });
